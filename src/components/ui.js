...
const UI = {
      ...
      <div class="container">
        <div class="footer-grid">
          <div class="footer-brand">
            ...
            <div class="socials"><a href="${SITE.social.facebook}" target="_blank" rel="noopener" aria-label="${t("followFb")}">${icon("facebook")}</a></div>
          </div>
          ...
          <div class="footer-col">
            ...
            <ul class="contact-lines">
              ...
              <li>${icon("mail")}${val(SITE.email)}</li>
  ...
  newsCard(n) {
    return `<article class="card news-card" data-rv>
      ${UI.media({ src: n.image, alt: "" , icon: "image" })}
      <div class="card-body">
        <div class="news-meta"><span class="chip chip--primary">${t("newsLabel")}</span>${UI.newsDate(n)}</div>
        <h3 class="card-title"><a class="card-link" href="${link("/news/" + n.slug)}">${esc(n.title)}</a></h3>
        <p class="card-text">${esc(n.excerpt)}</p>
        <div class="card-foot"><span class="muted" style="font-size:var(--fs-xs)">${t("source")}: ${esc(n.source.name)}</span><span class="link-arrow card-cta">${t("read")}${icon("arrow")}</span></div>
      </div>
    </article>`;
  },
  ...
  ctaBand() {
    return `<section class="section" style="padding-bottom:0"><div class="container"><div class="cta-band" data-rv>
      ...
      <div style="position:relative"><h2>${tr(L("ابدأ خطوتك الأولى نحو رعاية صحية أفضل", "Take your first step towards better care"))}</h2><p>${tr(L("اختر التخصص والطبيب والموعد المناسب، ثم اتصل بنا مباشرة لتأكيد حجزك.", "Choose a specialty, doctor and preferred time — then call us directly to confirm your booking."))}</p></div>
