...
/* Image URL: uses Netlify Image CDN (auto WebP/AVIF + resize) in production */
const imgSrc = (path, w = 1200) => {
  if (!path) return null;
  if (typeof INLINE_MEDIA !== "undefined" && INLINE_MEDIA[path]) return INLINE_MEDIA[path];
  if (BUILD.imageCdn && path.startsWith("/")) return `/.netlify/images?url=${encodeURIComponent(path)}&w=${w}&q=80`;
