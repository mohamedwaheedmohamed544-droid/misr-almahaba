...
const DoctorsPage = {
  ...
  render() {
    return `${UI.pageHero({ title: L("دليل الأطباء", "Doctor Directory"), lead: L("ابحث بالاسم أو التخصص، واطّلع على مواعيد الطبيب، ثم اتصل بنا للحجز.", "Search by name or specialty, check the doctor’s hours, then call us to book."), crumbs: [{ label: t("doctors") }],
      extra: `<div class="notice" style="max-width:720px">${icon("info")}<span>${tr(L("الملفات المعروضة <b>بيانات تجريبية</b> لتوضيح التجربة، وتُستبدل بقائمة الأطباء المعتمدة من إدارة المستشفى.", "Profiles shown are <b>sample data</b> to demonstrate the experience and will be replaced by the hospital’s approved roster."))}</span></div>` })}
    ...
    <section class="section" style="padding-top:var(--sp-6)"><div class="container detail">
      <div class="profile">
        ...
        <div class="profile-head">
          ...
          <div class="prose">
            <h2>${tr(L("نبذة", "About"))}</h2><p>${esc(d.bio)}</p>
            <h2>${tr(L("المؤهلات", "Qualifications"))}</h2>${d.qualifications.length ? `<ul>${d.qualifications.map((q) => `<li>${esc(q)}</li>`).join("")}</ul>` : `<p>${val(TBD(L("المؤهلات العلمية والخبرات", "Qualifications and experience")))}</p>`}
...
const NewsPage = {
  ...
  render() {
    ...
    <section class="section" style="padding-top:var(--sp-6)"><div class="container">
      <div class="grid grid-3">${NEWS.map(UI.newsCard).join("")}</div>
      ...
      ${UI.empty(tr(L("لا توجد مقالات منشورة بعد. تابع صفحتنا على فيسبوك لآخر المستجدات.", "No articles published yet. Follow our Facebook page for updates.")), `<a class="btn btn--soft btn--sm" href="${SITE.social.facebook}" target="_blank" rel="noopener">${icon("facebook")}Facebook</a>`)}
    </div></section>`;
  },
};
...
const NewsDetailPage = {
  ...
  render({ slug }) {
    ...
    return `${UI.pageHero({ title: n.title, crumbs: [{ href: link("/news"), label: t("news") }, { label: t("newsLabel") }],
      extra: `<div class="news-meta"><span class="chip chip--primary">${t("newsLabel")}</span>${UI.newsDate(n)}<span>${t("source")}: ${esc(n.source.name)}</span></div>` })}
    <section class="section" style="padding-top:var(--sp-5)"><div class="container"><article class="article">
      ${UI.media({ src: n.image, alt: L("مبنى مستشفى مصر المحبة", "Misr Al-Mahaba Hospital building"), eager: true })}
      <div class="prose" style="max-width:none">${n.body.map((p) => `<p>${esc(p)}</p>`).join("")}</div>
      <div class="source-box"><span>${tr(L("ملخص لخبر منشور في", "Summary of a story published by"))} «${esc(n.source.name)}»</span><a class="btn btn--soft btn--sm" href="${n.source.url}" target="_blank" rel="noopener">${tr(L("اقرأ المصدر الأصلي", "Read the original"))}${icon("external")}</a></div>
  ...
  render() {
    ...
    <section class="section" style="padding-top:var(--sp-6)"><div class="container contact-grid">
      <div class="contact-cards">
        ...
        <div class="contact-card" data-rv><div class="icon-tile">${icon("facebook")}</div><div><b>${tr(L("تابعنا وراسلنا", "Follow & message us"))}</b><p><a href="${SITE.social.facebook}" target="_blank" rel="noopener">facebook.com/MisrAlmahaba</a></p><p><a href="https://m.me/MisrAlmahaba" target="_blank" rel="noopener">${tr(L("راسلنا عبر Messenger", "Message us on Messenger"))}</a></p><p>${tr(L("البريد", "Email"))}: ${val(SITE.email)}</p></div></div>
      </div>
      <a class="map-box" href="${SITE.mapsUrl}" target="_blank" rel="noopener" data-rv="mask" aria-label="${t("openMaps")}">
        <img src="${IMG.buildingFull}" alt="" loading="lazy">
