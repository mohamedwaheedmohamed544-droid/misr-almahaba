...
const BookingPage = {
  ...
  async mount(root) {
    ...
    const canNext = () => [!!S.dept, !!S.doctor, true][S.step];
    ...
    const views = {
      ...
      1: async () => {
        ...
        <div class="notice" style="margin-bottom:var(--sp-4)">${icon("info")}<span>${tr(L("الأطباء المعروضون بيانات تجريبية لحين اعتماد القائمة الرسمية.", "Doctors shown are sample data until the official roster is approved."))}</span></div>
        <div class="choice-grid" role="radiogroup" aria-label="${esc(STR.doctors)}">${docs.map((d) => `<button type="button" role="radio" class="choice" data-doctor="${d.id}" aria-checked="${S.doctor?.id === d.id}">${UI.media({ src: d.photo, sm: true, avatar: true })}<span><b>${esc(d.name)}</b><small>${esc(d.title)} · ${d.schedule.map((s) => tr(DAY_NAMES[s.day])).join(LANG === "ar" ? "، " : ", ")}</small></span></button>`).join("")}</div>${nav()}`;
      },
      2: () => {
