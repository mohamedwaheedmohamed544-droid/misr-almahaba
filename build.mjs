...
if (existsSync(u("admin/"))) {
  ...
  // Repo resolution order: CMS_REPO env → GITHUB_REPO in admin/config.js → Netlify REPOSITORY_URL → git remote
  ...
  const repo = process.env.CMS_REPO || pinned || fromUrl(process.env.REPOSITORY_URL) || fromUrl(gitRemote) || "";
  if (repo) console.log(`✔ dashboard repo: ${repo}`);
  else console.warn("⚠ dashboard repo not detected — set GITHUB_REPO in admin/config.js or CMS_REPO in Netlify environment variables");
  const cfg = rawCfg.replace('const GITHUB_REPO = "' + (pinned || "") + '"', 'const GITHUB_REPO = "' + repo + '"').replace("__BRANCH__", process.env.CMS_BRANCH || process.env.HEAD || process.env.BRANCH || "main").replaceAll("__SITE_URL__", SITE_URL);
