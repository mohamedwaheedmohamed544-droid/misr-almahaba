...
> **مهم:** اكتب اسم المستودع في أول سطر من `admin/config.js`:
> `const GITHUB_REPO = "اسم-الحساب/misr-almahaba";`
> (أو أضف متغير `CMS_REPO` بنفس القيمة في Netlify ← Environment variables). البناء يحاول اكتشافه تلقائيًا، ويكتب في سجل النشر `✔ dashboard repo: ...` أو تحذيرًا إذا لم يجده.
